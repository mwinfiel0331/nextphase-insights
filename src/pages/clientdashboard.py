...
def show_client_dashboard(user_data: dict):
    ...
    try:
        ...
        with col2:
            # Add key parameter to button to ensure unique identification
            if st.button(button_text, key="start_intake_btn", type="primary"):
                # Set page in session state and trigger rerun
                st.session_state.page = "intake_form"
                st.session_state.form_section = 0  # Reset form section
                st.session_state.form_data = {}    # Clear any existing form data
                st.rerun()
            
    except Exception as e:
        logger.error(f"Error loading intakes: {str(e)}")
        st.error("Unable to load intakes. Please try again later.")